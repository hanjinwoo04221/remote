...
export class AgentService {
    static async run({ geminiApiKey, githubToken, owner, repo, branch, prompt, modelName = 'gemini-2.5-flash', onEvent, }) {
        const ai = new GoogleGenAI({ apiKey: geminiApiKey });
        const proposedChanges = new Map();
        // Cache repo tree for quick lookups
        let cachedTree = null;
        const getCachedTree = async () => {
            if (!cachedTree) {
                ...
                functionDeclarations: [
                    {
                        ...
                        description: 'Retrieve directory and file paths of the repository to understand project structure.',
                        parameters: {
                            type: Type.OBJECT,
                            properties: {
                                maxResults: {
                    ...
                    },
                    {
                        ...
                        description: 'Propose an edit to an existing file or propose creating a new file. Provide the COMPLETE new file content.',
                        parameters: {
                            type: Type.OBJECT,
                            properties: {
                                path: {
                                ...
                                newContent: {
                                    type: Type.STRING,
                                    description: 'The full, complete code of the file after the modification. Never truncate with comments like "...rest of code".',
                                },
                                description: {
                                    type: Type.STRING,
            ...
            for (const call of functionCalls) {
                ...
                try {
                    if (name === 'get_repo_structure') {
                        ...
                        const fileData = await GitHubService.getFileContent(githubToken, owner, repo, filePath, branch);
                        result = {
                            path: filePath,
                        ...
                        let action = 'create';
                        try {
                            const original = await GitHubService.getFileContent(githubToken, owner, repo, filePath, branch);
